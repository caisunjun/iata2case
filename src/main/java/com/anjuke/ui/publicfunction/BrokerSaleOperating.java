...
public class BrokerSaleOperating {
	...
	public void releaseSale(Browser driver, AnjukeSaleInfo saleInfo, boolean needPic) {
		...
		if (driver.check(Init.G_objMap.get("ajk_releaseSale_div_full"))) {
			...
			/*
			 * String propInfoTitle = driver.getText(Init.G_objMap.get(
			 * "anjuke_wangluojingjiren_sale_detail_title"),"获取房源标题");
			 */
...
//		}
	...
	public void editSale(Browser driver, AnjukeSaleInfo updateInfo, boolean needPic) throws InterruptedException {
		...
		// driver.click(Init.G_objMap.get("anjuke_wangluojingjiren_sale_update_firstEdit"),
		// "编辑第一条数据");
		...
		// 装修情况-------------------------------------------新版发房ie6还没改 --ie6 must
		// die
		// XPATH 适配IE6不行 使用id定位IE6
		// driver.click("id^radFitMent_1", "装修情况：普通装修");
		// driver.click(Init.G_objMap.get("anjuke_wangluojingjiren_sale_fitMent1_ie6"),
		// "装修情况：普通装修");
		driver.select(Init.G_objMap.get("anjuke_wangluojingjiren_sale_fitMent"), updateInfo.getFitmentInfo());

		// 朝向-------------------------------------------新版发房ie6还没改 --ie6 must
		// die
		// XPATH 适配IE6不行 使用id定位IE6
		// driver.click("id^radHouseOri_4", "方向：东南");
		// driver.click(Init.G_objMap.get("anjuke_wangluojinjir_sale_chaoxiang_southeast_ie6"),
		// "方向：东南");
		...
		// String propInfoTitle = driver.getText("className^propInfoTitle",
		// "获取房源标题");
		// //String propInfoTitle =
		// driver.getText(Init.G_objMap.get("anjuke_wangluojingjiren_sale_detail_title"),
		// "获取房源标题");
		// if(propInfoTitle.equals(updateInfo.getHouseTitle())){
		// Report.writeHTMLLog("房源详细页", "验证房源详细页标题-->"+propInfoTitle,
		// Report.PASS, "");
		// }else{
		// Report.writeHTMLLog("房源详细页",
		// "验证房源详细页标题错误-->"+propInfoTitle+"**正确的应该为："+updateInfo.getHouseTitle(),
		// Report.FAIL, driver.printScreen());
		// }
		// //税费自理价格检查
		// String taxPrice =
		// driver.getText(Init.G_objMap.get("anjuke_wangluojingjiren_sale_detail_taxPrice"),
		// "获取税费自理价");
		...
		// }else{
		// Report.writeHTMLLog("房源详细页",
		// "验证税费自理价错误-->"+taxPrice+"万**应该为："+updateInfo.getPriceTaxe()+"万",
		// Report.FAIL, driver.printScreen());
		// }
		// //房屋单价检查
		// String unitPrice =
		// driver.getText(Init.G_objMap.get("anjuke_wangluojingjiren_sale_detail_price"),
		// "获取房屋单价");
		// if(unitPrice.equals(updateInfo.getPrice()+"/平米")){
		// Report.writeHTMLLog("房源详细页", "验证房屋单价-->"+unitPrice ,Report.PASS, "");
		// }else{
		// Report.writeHTMLLog("房源详细页",
		// "验证房屋单价错误-->"+unitPrice+"**应该为："+updateInfo.getPrice()+"/平米",
		// Report.FAIL, driver.printScreen());
		// }
		// //检查房型
		// String houseType =
		// driver.getText(Init.G_objMap.get("anjuke_wangluojingjiren_sale_detail_housetype"),
		// "获取房型");
		// String updateInfo_houseT =
		// "房型："+updateInfo.getHouseType_S()+"室"+updateInfo.getHouseType_T()+"厅"+updateInfo.getHouseType_W()+"卫";
		// if(houseType.equals(updateInfo_houseT)){
		// Report.writeHTMLLog("房源详细页", "验证房型详细信息-->"+houseType, Report.PASS,
		// "");
		// }else{
		// Report.writeHTMLLog("房源详细页",
		// "验证房型详细信息错误-->"+houseType+"正确为："+updateInfo_houseT, Report.FAIL,
		// driver.printScreen());
		// }
		// //产证面积
		// String houseArea =
		// driver.getText(Init.G_objMap.get("anjuke_wangluojingjiren_sale_detail_area"),
		// "产证面积");
		// if(houseArea.equals("面积："+updateInfo.getHouseArea()+"平米")){
		// Report.writeHTMLLog("房源详细页", "验证产证面积-->"+houseArea, Report.PASS, "");
		// }else{
		// Report.writeHTMLLog("房源详细页",
		// "验证产证面积错误-->"+houseArea+"正确为："+"产证面积："+updateInfo.getHouseArea(),
		// Report.FAIL, driver.printScreen());
		// }
		// //小区名称
		// String commuName =
		// driver.getAttribute("//a[@id='text_for_school_1']", "title");
		// if(commuName.equals(updateInfo.getCommunityName())){
		// Report.writeHTMLLog("房源详细页", "验证小区名称-->"+commuName, Report.PASS, "");
		// }else{
		// Report.writeHTMLLog("房源详细页",
		// "验证小区名称错误-->"+commuName+"正确为："+updateInfo.getCommunityName(),
		// Report.FAIL, driver.printScreen());
		// }
		// //朝向
		// String houseDire =
		// driver.getText(Init.G_objMap.get("anjuke_wangluojingjiren_sale_detail_dire"),
		// "房子朝向");
		// if(houseDire.equals("朝向："+updateInfo.getOrientations())){
		// Report.writeHTMLLog("房源详细页", "验证朝向-->"+houseDire, Report.PASS, "");
		// }else{
		// Report.writeHTMLLog("房源详细页",
		// "验证朝向错误-->"+houseDire+"正确为："+updateInfo.getOrientations(),
		// Report.FAIL, driver.printScreen());
		// }
		// //建筑年代
		// String buildYear =
		// driver.getText(Init.G_objMap.get("anjuke_wangluojingjiren_sale_detail_year"),
		// "建筑年代");
		// if(buildYear.equals("建造年代："+updateInfo.getBuildYear()+"年")){
		// Report.writeHTMLLog("房源详细页", "验证建筑年代-->"+buildYear, Report.PASS, "");
		// }else{
		// Report.writeHTMLLog("房源详细页",
		// "验证建筑年代错误-->"+buildYear+"正确为："+updateInfo.getBuildYear()+"年",
		// Report.FAIL, driver.printScreen());
		// }
		// //楼层
		// String floor =
		// driver.getText(Init.G_objMap.get("anjuke_wangluojingjiren_sale_detail_floor"),
		// "楼层");
		// String floorInfo =
		// "楼层："+updateInfo.getFloorCur()+"/"+updateInfo.getFloorTotal();
		// if(floor.equals(floorInfo)){
		// Report.writeHTMLLog("房源详细页", "验证楼层情况-->"+floor, Report.PASS, "");
		// }else{
		// Report.writeHTMLLog("房源详细页", "验证楼层情况-->"+floor+"**正确为："+floorInfo,
		// Report.FAIL, driver.printScreen());
		// }
		// //装修
		// String fitment =
		// driver.getText(Init.G_objMap.get("anjuke_wangluojingjiren_sale_detail_fitment"),
		// "装修");
		// if(fitment.equals("装修："+updateInfo.getFitmentInfo())){
		// Report.writeHTMLLog("房源详细页", "验证装修-->"+fitment, Report.PASS, "");
		// }else{
		// Report.writeHTMLLog("房源详细页",
		// "验证装修-->"+fitment+"正确为："+updateInfo.getFitmentInfo(), Report.FAIL,
		// driver.printScreen());
		// }
		// //房源描述
		// String description =
		// driver.getText(Init.G_objMap.get("anjuke_wangluojingjiren_sale_detaile_desc"),
		// "房源描述");
		...
		// }else{
		// Report.writeHTMLLog("房源详细页",
		// "验证房源描述-->"+description+"正确为："+updateInfo.getHouseDescribe(),
		// Report.FAIL, driver.printScreen());
		// }
		...
		// driver.get("http://my.anjuke.com/user/brokerpropmanage/W0QQactZsale#proptop");
		// if(driver.check(Init.G_objMap.get("anjuke_wangluojingjiren_sale_update_firestsele"))&&driver.getText(Init.G_objMap.get("anjuke_wangluojingjiren_sale_list_firstTitle"),
		// "获取房源名称").equals(updateInfo.getHouseTitle())){
		// driver.click(Init.G_objMap.get("anjuke_wangluojingjiren_sale_update_firestsele"),
		// "选中发布房源");
		// driver.click(Init.G_objMap.get("anjuke_wangluojingjiren_sale_hideprop"),
		// "删除发布房源");
		// }else{
		// Report.writeHTMLLog("删除房源", "没有可以删除的房源", "DONE",
		// driver.printScreen());
		// }
	}
