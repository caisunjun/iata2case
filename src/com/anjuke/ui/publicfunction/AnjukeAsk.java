...
public class AnjukeAsk {
	...
	public static String submitQuestion(Browser driver, String question_title,
		...
		if (!driver.getAttribute(Ajk_AskQuestion.OPENMORE, "class").equals(
				"forclose")) {
			driver.click(Ajk_AskQuestion.OPENMORE, "点击中间的展开条");
		}
		driver.type(Ajk_AskQuestion.DISCRIPTION, question_description,
				"输入问题的描述");
		...
		if (driver.check(Ajk_AskQuestionSuccess.VIEWBUTTON)) {
			question_href = driver.getAttribute(
					Ajk_AskQuestionSuccess.VIEWBUTTON, "href");
			driver.click(Ajk_AskQuestionSuccess.VIEWBUTTON, "点击查看按钮进入问答单页");
			Report.writeHTMLLog("张璐的测试报告之提问是否成功", "提问成功且问题URL是："
					+ question_href, Report.PASS, "");
		} else {
			String ps = driver.printScreen();
			Report.writeHTMLLog("张璐的测试报告之提问是否成功", "提问失败，没有进入提问成功页",
					Report.FAIL, ps);
		}
	...
	public static void submitAnswer(Browser driver, String content) {
		...
		n = 0;
		boolean relate = true;
		if (driver.check(Ajk_AskView.RELATED)
				&& driver.getText(Ajk_AskView.RELATED, "获取相关问题").equals("相关问题")) {
		...
		if (driver.getAttribute(Ajk_AskView.IANSWER, "class").equals(
				"answer-btn down")) {
		...
		if (n > m) {
			for (int i = 1; i <= n;) {
				...
				String actual = driver.getText(Ajk_AskView.getAnswerElement(i,
						relate), "取第i条回复的数据");
				if (actual.equals(content)) {
					Report.writeHTMLLog("问答单页判断回答是否成功",
							"问答单页回答列表中有一条数据与实际值是匹配的", Report.PASS, "");
				...
				if (i == n + 1) {
					Report.writeHTMLLog("问答单页判断回答是否成功",
							"回答失败：问答单页回答列表中没有一条数据与实际值是匹配的:" + "^预期值" + content
									+ "^实际值" + actual, Report.FAIL, "");
	...
	public static void submitSupplement(Browser driver, String addContent) {
		...
		driver.assertEquals(addContent, getAskViewDescriptionAdd(driver),
				"检测补充问题是否成功", "检测补充问题内容是否与实际一致");
	}
	...
	public static void checkNormalUserAskInfo(Browser driver, String expertTitle) {
		...
		driver.assertNonEquals(null, getNormalUserExperience(driver),
				"个人中心提问列表的数据检测", "检测经验值是否为空");
		driver.assertNonEquals(null, getNormalUserGold(driver),
				"个人中心提问列表的数据检测", "检测金币值是否为空");
		driver.assertNonEquals(null, getNormalUserAskNum(driver),
				"个人中心提问列表的数据检测", "检测提问数是否为空");
		driver.assertNonEquals(null, getNormalUserAnswerNum(driver),
				"个人中心提问列表的数据检测", "检测回答数是否为空");
		driver.assertNonEquals(null, getNormalUserAdoptionRate(driver),
				"个人中心提问列表的数据检测", "检测采纳率是否为空");
		driver.assertEquals(expertTitle,
				getNormalUserMyQuestionListFirst(driver), "个人中心提问列表的数据检测",
				"检测最新提问的问答标题是否正确--即最新的提问是否被记录");
		/*
	...
	public static void checkAskSearch(Browser driver, String key) {
		...
		if (key.trim().equals("") == true) {
			driver.assertEquals("所有问题", driver.getText(Ajk_Ask.CRUMBS_KEY,
					"获取面包屑上的关键字"), "检测异常搜索结果页", "检测异常搜索结果页面包屑上的关键字是否匹配");
			...
			if (getNormalSearchNoResults(driver).contains(key)) {
				Report.writeHTMLLog("问答首页搜索关键字用例", "判断正常搜索无结果时是否正确显示友情提示语",
						Report.PASS, "");
			} else {
				Report.writeHTMLLog("问答首页搜索关键字用例", "判断正常搜索无结果时是否正确显示友情提示语",
						Report.FAIL, "");
			...
			for (int i = 0; i < m;) {
				...
				if (actualContent.contains(keys[i])) {
					Report.writeHTMLLog("问答首页搜索关键字用例", "判断正常搜索结果是否模糊匹配搜索关键字",
							Report.PASS, "");
					break;
				} else
					i++;
	...
	public static String getSearchHighlightKey(Browser driver) {
		...
		int i;
		for (i = 1; i <= n; i++) {
			partKey[i - 1] = driver.getText(Ajk_Ask.HIGHLIGHT_KEY + "[" + i
					+ "]", "获取高亮的所有关键字");
	...
	public static String getListsAnyoneTitle(Browser driver) {
		...
		if (m == 12) {
			Report.writeHTMLLog("检测问答搜索列表页在有很多数据的情况下第一页最多展示条数",
					"检测问答搜索列表页在有很多数据的情况下第一页最多展示条数是否为12条", Report.PASS, "p1");
		} else {
			Report.writeHTMLLog("检测问答搜索列表页在有很多数据的情况下第一页最多展示条数",
					"检测问答搜索列表页在有很多数据的情况下第一页最多展示条数是否为12条", Report.FAIL, "p2");
		}
	...
	public static void EditExternalExpertInfo(Browser driver, String trueName,
		...
		String SubmitSuccessTitle = driver.getText(
				Ajk_AskExpertView.SubmitSuccess, "获取编辑资料成功后的提示语");
		System.out.println(SubmitSuccessTitle);
		driver.assertEquals("您的资料已更新成功，立刻去个人主页    去看", SubmitSuccessTitle,
				"判断外部专家的资料编辑是否修改成功", "");
	}
	...
	public static void checkExpertMovingData(Browser driver) {
		...
		// "检测问答右侧专家模块数据是否展示3条");
		if (n == 3) {
			Report.writeHTMLLog("检测问答右侧专家模块数据是否展示3条", "检测问答右侧专家模块数据是否展示3条",
					Report.PASS, "p3");
		} else {
			Report.writeHTMLLog("检测问答右侧专家模块数据是否展示3条", "检测问答右侧专家模块数据是否展示3条",
					Report.FAIL, "p4");
		...
		for (int i = 1; i <= n; i++) {
			String ExpertMovingTest = driver.getText(Ajk_Ask.ExpertMoving + "["
					+ i + "]", "获取每条数据的文本");
			driver.assertNonEquals(null, ExpertMovingTest, "检测专家动态模块的数据是否为空",
					"检测专家动态模块的数据是否为空");
		}
	}
	...
	public static void checkAskHomeToBeResolveQuestion(Browser driver) {
		...
		String conURL = "jdbc:mysql://10.11.8.54:3306/user_db?useUnicode=true&characterEncoding=utf8"; // MySQL数据库Company的URL
		Connection con = null;
		try {
