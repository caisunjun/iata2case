...
/*		try {
		...
		*/
		...
		for(int i=1;i<=n1;i++){
			String QuestionData = driver.getText(fiveTabElementList[tab]+"["+i+"]", "获取第"+tab+"个TAB下每条数据的文本");
			String QuestionExpertName = driver.getText(fiveTabElementList[tab]+"["+i+"]/div[1]/a", "获取第"+tab+"个TAB下每条数据对应专家姓名的文本");
			System.out.println("每条数据的文本内容如下："+QuestionData);
			System.out.println("每条数据下对应专家的姓名："+QuestionExpertName+"\n");
			driver.assertNonEquals(null, QuestionData, "检测第"+tab+"个TAB下每条数据是否为空", "检测第"+tab+"个TAB下每条数据是否为空");
			driver.assertNonEquals(null, QuestionExpertName, "检测第"+tab+"个TAB下专家姓名是否为空", "检测第"+tab+"个TAB下专家姓名是否为空");
		}
