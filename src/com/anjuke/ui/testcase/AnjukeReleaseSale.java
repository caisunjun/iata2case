 ...
 */
public class AnjukeReleaseSale {
...
//			driver.exitFrame();
			...
			driver.type(Init.G_objMap.get("anjuke_wangluojingjiren_sale_fangyuanbiaoti_name"),"发布出售请勿联系谢谢" + time, "房源标题");
			saleInfo.setHouseTitle("发布出售请勿联系谢谢" + time);
			// 房源描述
			driver.switchToIframe(Init.G_objMap.get("anjuke_wangluojingjiren_salse_ifrExplain"),
					"获取房源描述frame");
...
//					"测试房源，请勿联系，谢谢", "房源备注");
			...
			driver.click(Init.G_objMap.get("anjuke_wangluojingjiren_sale_tuijiangaifangyuan"),
					"推荐该房源，在二手房列表页显示");
...
//			driver.switchWindo(2);
			...
			if (propInfoTitle.equals(saleInfo.getHouseTitle())) {
				...
				Report.writeHTMLLog("房源详细页", "房源详细页标题错误：" + propInfoTitle
						+ "**正确的应该为：" + saleInfo.getHouseTitle(), Report.FAIL,
						driver.printScreen());
			}
			// 税费自理价格检查
			String taxPrice = driver.getText(Init.G_objMap
					.get("anjuke_wangluojingjiren_sale_detail_taxPrice"),
					"获取税费自理价");
			if (taxPrice.equals(saleInfo.getPriceTaxe())) {
				Report.writeHTMLLog("房源详细页", "税费自理价：" + taxPrice + "万",
						Report.PASS, "");
			} else {
				Report.writeHTMLLog("房源详细页", "税费自理价错误：" + taxPrice + "万"
						+ "**应该为：" + saleInfo.getPriceTaxe() + "万",
						Report.FAIL, driver.printScreen());
			}
			// 房屋单价检查
			...
			if (unitPrice.equals(saleInfo.getPrice() + "/平米")) {
				Report.writeHTMLLog("房源详细页", "验证房屋单价-->" + unitPrice,
						Report.PASS, "");
			} else {
				Report.writeHTMLLog("房源详细页", "验证房屋单价错误-->" + unitPrice
						+ "**应该为：" + saleInfo.getPrice() + "/平米", Report.FAIL,
						driver.printScreen());
			}
			// 房东到手价检查  页面无房东到手价
			...
			String houseType = driver.getText(Init.G_objMap
					.get("anjuke_wangluojingjiren_sale_detail_housetype"),
					"获取房型");
			String saleInfo_houseT = "房型：" + saleInfo.getHouseType_S()
					+ saleInfo.getHouseType_T() + saleInfo.getHouseType_W();
			if (houseType.equals(saleInfo_houseT)) {
				...
				Report.writeHTMLLog("房源详细页", "房型详细信息错误：" + houseType + "正确为："
						+ saleInfo_houseT, Report.FAIL, driver.printScreen());
			}
			// 产证面积
			String houseArea = driver.getText(Init.G_objMap
					.get("anjuke_wangluojingjiren_sale_detail_area"), "产证面积");
			if (houseArea.equals("面积：" + saleInfo.getHouseArea())) {
				Report.writeHTMLLog("房源详细页", "产证面积：" + houseArea, Report.PASS,
						"");
			} else {
				Report.writeHTMLLog("房源详细页", "产证面积错误：" + houseArea + "正确为："
						+ "产证面积：" + saleInfo.getHouseArea(), Report.FAIL,
						driver.printScreen());
			}
			// 朝向
			...
			if (houseDire.equals(saleInfo.getOrientations())) {
				...
				Report.writeHTMLLog("房源详细页", "朝向错误：" + houseDire + "正确为："
						+ saleInfo.getOrientations(), Report.FAIL,
						driver.printScreen());
			}
			// 建筑年代
			...
			if (buildYear.equals(saleInfo.getBuildYear() + "年")) {
				...
				Report.writeHTMLLog("房源详细页", "建筑年代错误：" + buildYear + "正确为："
						+ saleInfo.getBuildYear() + "年", Report.FAIL,
						driver.printScreen());
			}
			// 楼层
			String floor = driver.getText(Init.G_objMap
					.get("anjuke_wangluojingjiren_sale_detail_floor"), "楼层");
			String floorInfo = "楼层：" + saleInfo.getFloorCur() + "/"
					+ saleInfo.getFloorTotal();
			...
			if (fitment.equals(saleInfo.getFitmentInfo())) {
				...
				Report.writeHTMLLog("房源详细页", "装修：" + fitment + "正确为："
						+ saleInfo.getFitmentInfo(), Report.FAIL,
						driver.printScreen());
			}
			// 房源描述
			...
			if (description.equals(saleInfo.getHouseDescribe())) {
				...
				Report.writeHTMLLog("房源详细页", "房源描述：" + description + "正确为："
						+ saleInfo.getHouseDescribe(), Report.FAIL,
						driver.printScreen());
			}
