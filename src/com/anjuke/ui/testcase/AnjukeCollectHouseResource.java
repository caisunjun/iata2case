...
/**
 * 该测试用例主要完成安居客收藏房源操作，逻辑如下 1、选择二手房列表页面，第一个房源 2、判断该房源是不是爱房的房源，如果是选择下一条数据
 * 3、在房源详细页进行收藏操作 4、提示收藏成功后，在房源列表页进行房源验证 5、当房源数据超过10条时，删除房源数据
 * 6、判断房源列表当前页，是否存在多条相同的数据
 * 
 * @UpdateAuthor Gabrielgao
 * @last updatetime 2012-05-08 11:30
 * @UpdateAuthor ccyang
 * @last updatetime 2012-08-01 10:30 列表页里经纪人名称模块，可能取到其他信息，需要将经纪人的名称取出来
 */
	...
	public void collectHouse() {
		...
		if (brow.contains("ie")) {
			...
			if (!(loginName == null || loginName.equals(""))) {
				...
				Report.writeHTMLLog("user login", "user login sucess,and name is: " + loginName,
						Report.DONE, "");
			} else {
				String ps = driver.printScreen();
				Report.writeHTMLLog("user login", "user login fail,and name is: " + loginName,
						Report.FAIL, ps);
			}
		}
		// 访问二手房列表
		...
		if (!driver.check(Init.G_objMap.get("sale_text_no_found"))) {
			...
			for (int i = 1; i <= count; i++) {
				String anjuke_salefirstdata_title = Init.G_objMap
						.get("anjuke_saleresultlist_firstdata") + i;
				String anjuke_salefirstdata_price = Init.G_objMap.get("anjuke_salefirstdata_price")
						+ i + "']/strong";
				String anjuke_salefirstdata_releaseName = Init.G_objMap
						.get("anjuke_salefirstdata_releaseName") + i + "']/div[2]/p[2]";
				String anjuke_salefirstdata_img = Init.G_objMap.get("anjuke_salefirstdata_img") + i
						+ "_a";
				...
				if (!url.contains("aifang")) {
					...
					// 经纪人姓名的控件里可能出现其他内容
					releaseName = driver.getText(anjuke_salefirstdata_releaseName,
							"获取第一套合格的房源经纪人姓名").trim();
				...
				if (!driver.check(Init.G_objMap.get("anjuke_sale_member_houselist_noresult"))) {
					...
					if (collectPrice.equals(price)) {
						Report.writeHTMLLog("collect data price", "collect data price is correct!"
								+ collectPrice + "万" + "<br>VS " + price + "万", Report.PASS, "");
					} else {
						String ps = driver.printScreen();
						Report.writeHTMLLog("collect data price", "collect data price is fault",
								Report.FAIL, ps);
					}
					// 判断选房单发布人姓名
					...
					if (collectReleName.equals(releaseName)) {
						Report.writeHTMLLog("collect data releaseName",
								"collect data releaseName is correct!" + collectReleName
										+ "<br>VS " + releaseName, Report.PASS, "");
					} else {
						String ps = driver.printScreen();
						Report.writeHTMLLog("collect data releaseName",
								"collect data releaseName is fault!" + collectReleName + "<br>VS"
										+ releaseName, Report.FAIL, ps);
					}
					// 判断是否存在相同的数据
					int datacount = driver.getElementCount(Init.G_objMap
							.get("anjuke_sale_member_houselist_dataCount"));
					int num = Integer.parseInt(driver.getText(
							Init.G_objMap.get("anjuke_sale_member_houselist_textNum"), "获取房源总数"));
					if (datacount != 1) {
						boolean status = true;
						...
						for (int i = 1; i < datacount; i++) {
							...
							if (driver.check("//*[@id='prop_name_qt_apf_id_" + tmp + "']", 1)) {
								hrefResult = driver.getAttribute("//*[@id='prop_name_qt_apf_id_"
										+ tmp + "']", "href");
							} else {
								tmp = tmp + 2;
								hrefResult = driver.getAttribute("//*[@id='prop_name_qt_apf_id_"
						...
						if (collectProp > 0) {
							driver.click("//*[@id='apf_id_" + collectProp + "']/div[2]/div[2]/a",
									"删除收藏的房源");
					...
					if (num >= 6) {
						...
						if (driver.check("//div[@class='deletespan']", 10)) {
							resultNum = Integer.parseInt(driver.getText(
									Init.G_objMap.get("anjuke_sale_member_houselist_textNum"),
									"获取房源总数"));
						}
						if (resultNum < num) {
