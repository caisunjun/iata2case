...
/**
 * 该测试用例主要完成保存搜房条件操作，逻辑如下 1、访问二手房列表页面 2、选择找房条件 3、点击订阅该找房条件
 * 4、判断该找房条件是否存在，如果存在则删除找房条件，并且在此订阅 5、如果条件不存在，则保存成功后，进入条件列表页，验证保存是否成功
 * 
 * @author Gabrielgao
...
public class AnjukeSaveFindHouseCondition {
	...
	public void saveCon(boolean lastCon) {
		...
		// driver.click(Init.G_objMap.get("anjuke_sale_condition_type"),
		// "选择类型");
	...
	private boolean equalCondition(String searchCon, String split) {
		...
		// 空格
		if (split.equals("[")) {
			lastCon = "[ " + condition.get("range") + " " + condition.get("price") + " "
					+ condition.get("area") + " " + condition.get("room") + " "
					+ condition.get("type") + " ]";
		}
		// 逗号
		if (split.equals("，")) {
		...
		if (split.equals("top")) {
			// lastCon =
			// condition.get("range")+condition.get("price")+condition.get("area")+condition.get("room")+condition.get("type")+"二手房";
			lastCon = condition.get("range") + condition.get("price") + condition.get("area")
					+ condition.get("room") + condition.get("type") + "二手房";
		}
		...
		if (searchCon.equals(lastCon)) {
			...
			String ps = driver.printScreen();
			Report.writeHTMLLog("比较保存找房条件", "找房条件与保存不一致<br>" + searchCon + "<br>" + lastCon,
					Report.FAIL, ps);
			return false;
		}
	}
	...
	private void verifyCondition() {
		...
		if (count != 0) {
			for (int i = 1; i <= count; i++) {
				String con_top = driver.getText("//form[@id='manacondform']/table/tbody/tr["
						+ (i * 2) + "]/td[@class='cond']/div/a", "获取上方的条件");
			...
			if (equalCount > 1) {
				String ps = driver.printScreen();
				Report.writeHTMLLog("订阅管理-找房条件", "保存找房条件失败，存在相同的找房条件共:" + equalCount + "条",
						Report.FAIL, ps);
			}
			if (equalCount == 0) {
				String ps = driver.printScreen();
